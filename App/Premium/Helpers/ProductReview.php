...
class ProductReview extends Base
{
    ...
    function get_orders_ids_by_product_id($product_id, $email, $limit = 1)
    {
        ...
        LEFT JOIN {$wpdb->prefix}woocommerce_order_itemmeta as woim1 ON woi.order_item_id = woim1.order_item_id AND woim1.meta_key = %s
        LEFT JOIN {$wpdb->prefix}posts as p ON woi.order_id = p.ID LEFT JOIN {$wpdb->prefix}postmeta as pm ON p.ID = pm.post_id
        WHERE p.post_status IN ( $earning_status ) AND woim.meta_key IN ( %s, %s ) 
          AND woim.meta_value = %s AND pm.meta_key= %s AND pm.meta_value = %s AND woim1.meta_value IS NULL  ORDER BY woi.order_item_id DESC LIMIT %d",
            array('_bundled_by', '_product_id', '_variation_id', $product_id, '_billing_email', $email, $limit));
        return self::$number_order_made_with_product_ids[$email][$product_id] = $wpdb->get_col($query);
    }
