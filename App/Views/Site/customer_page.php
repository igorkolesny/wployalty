                    ...
                    <?php if (isset($campaign->is_show_way_to_earn) && $campaign->is_show_way_to_earn == 1): ?>
                        <div class="wlr-card wlr-earning-option wlr-border-color">
                            ...
                            <?php if (isset($campaign->action_type) && $campaign->action_type == 'birthday') : ?>
                                ...
                                $date_format = implode($date_format_orders['separator'], $date_format_orders['format']);
                                $birth_date = isset($user->birthday_date) && !empty($user->birthday_date) && $user->birthday_date != '0000-00-00' ? $woocommerce_helper->beforeDisplayDate(strtotime($user->birthday_date), $date_format) : (isset($user->birth_date) && !empty($user->birth_date) ? $woocommerce_helper->beforeDisplayDate($user->birth_date, $date_format) : '');
                                $wp_user = wp_get_current_user();
                                $user_can_edit_birthdate = (isset($user) && isset($user->id) && $user->id > 0) || (is_object($wp_user) && isset($wp_user->ID) && $wp_user->ID > 0);
                                $show_edit_birthday = apply_filters("wlr_allow_my_account_edit_birth_date", true, $user_can_edit_birthdate, isset($user) && !empty($user) ? $user : new \stdClass());
                                ?>
                            ...
                            <?php if (((isset($user) && is_object($user) && $user->id > 0) || get_current_user_id() > 0) && isset($campaign->action_type) && $campaign->action_type == 'followup_share') : ?>
                                ...
                                <div class="wlr-date"
                                     style="position:relative;<?php echo $is_right_to_left ? "float: left;" : "float:right;"; ?>">
                                    <a style="<?php echo !empty($theme_color) ? esc_attr("background:" . $theme_color . ";") : ""; ?>"
                                       class="wlr-button-text-color"
