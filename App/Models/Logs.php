...
class Logs extends Base
{
                ...
                if ($campaign_helper->is_valid_action($log_list->action_type) && in_array($log_list->action_process_type, array('earn_point', 'earn_reward', 'coupon_generated', 'reduce_point', 'email_notification'))) {
                    switch ($log_list->action_process_type) {
                        case 'earn_point':
                            ...
                            break;
                        case 'earn_reward':
                            $log_list->processed_custom_note = $log_list->action_type != 'achievement' ? sprintf(__('%s earned', 'wp-loyalty-rules'), $log_list->reward_display_name) : $log_list->customer_note;
                            break;
                        case 'coupon_generated':
                            $log_list->processed_custom_note = sprintf(__('%s generated from %s via %s', 'wp-loyalty-rules'), $log_list->discount_code, $log_list->reward_display_name, $campaign_helper->getActionName($log_list->action_type));
                            if (isset($log_list->required_points) && $log_list->required_points > 0) {
                                $log_list->processed_custom_note = sprintf(__('%s generated from %s via %s (%s used %s)', 'wp-loyalty-rules'), $log_list->discount_code, $log_list->reward_display_name, $campaign_helper->getActionName($log_list->action_type), $campaign_helper->getPointLabel($log_list->required_points), $log_list->required_points);
                            }
                            break;
                        case 'reduce_point':
                            ...
                            break;
                        case 'return_reward':
                            $log_list->processed_custom_note = sprintf(__('%s reward returned/expired', 'wp-loyalty-rules'), $log_list->reward_display_name);
                            break;
                    ...
                    if ($log_list->action_type == 'admin_change') {
                        switch ($log_list->action_process_type) {
                            ...
                            case 'birth_date_update':
                                ...
                                $log_list->processed_custom_note = sprintf(__('Birthdate change to %s', 'wp-loyalty-rules'), $user->birthday_date);
                                break;
                        }
                        ...
                        switch ($log_list->action_process_type) {
                            ...
                            case 'expire_email_and_date_change':
                                $log_list->processed_custom_note = sprintf(__('Updated expiry date %s for %s', 'wp-loyalty-rules'), $woocommerce_helper->beforeDisplayDate($log_list->expire_date, 'Y-m-d'), $log_list->reward_display_name);
                                break;
                        }
                    } elseif ($log_list->action_type == 'redeem_point') {
                        switch ($log_list->action_process_type) {
                            case 'coupon_generated':
                                $log_list->processed_custom_note = sprintf(__('%s coupon generated from %s', 'wp-loyalty-rules'), $log_list->discount_code, $log_list->reward_display_name);
                                if (isset($log_list->required_points) && $log_list->required_points > 0) {
                                    $log_list->processed_custom_note = sprintf(__('%s coupon generated from %s using %s %s', 'wp-loyalty-rules'), $log_list->discount_code, $log_list->reward_display_name, $log_list->required_points, $campaign_helper->getPointLabel($log_list->required_points));
                                }
                                break;
                            case 'earn_reward':
                                $log_list->processed_custom_note = sprintf(__('%s earned from %s', 'wp-loyalty-rules'), $log_list->reward_display_name, $campaign_helper->getPointLabel(3));
                                if (isset($log_list->required_points) && $log_list->required_points > 0) {
                                    $log_list->processed_custom_note = sprintf(__('%s earned from %s %s', 'wp-loyalty-rules'), $log_list->reward_display_name, $log_list->required_points, $campaign_helper->getPointLabel($log_list->required_points));
                                }
                                break;
                        }
    ...
    function saveLog($data)
    {
        ...
        if ($insert_data['action_type'] == 'expire_date_change'
            && ((isset($data['expire_email_date']) && !empty($data['expire_email_date']))) || (isset($data['expire_date']) && !empty($data['expire_date']))) {
            $insert_data['action_process_type'] = 'expire_email_and_date_change';
        }
