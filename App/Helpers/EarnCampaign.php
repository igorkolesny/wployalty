...
class EarnCampaign extends Base
{
    ...
    function processOrderReturn($order_id)
    {
        ...
        $status = get_post_meta($order_id, '_wlr_points_return_status', true);
        ...
        $status = get_post_meta($order_id, '_wlr_point_coupon_return_status', true);
        if (!$status) {
            ...
            foreach ($coupons as $coupon) {
                ...
                $discount_id = isset($coupon->discount_id) && $coupon->discount_id > 0 ? $coupon->discount_id : 0;
            ...
            update_post_meta($order_id, '_wlr_point_coupon_return_status', true);
        }
        update_post_meta($order_id, '_wlr_points_return_status', true);
    }
    ...
    function processOrderEarnPoint($order_id)
    {
        ...
        if (get_post_meta($order_id, '_wlr_points_earned_status', true)) {
        ...
        if ($this->applyEarnCampaign($action_data)) {
            ...
            update_post_meta($order_id, '_wlr_points_earned_status', true);
            return true;
        }
        return false;
    }
    ...
    function addEarnCampaignReward($action_type, $reward, $campaign_id, $action_data)
    {
                ...
                if (isset($campaign['rewards']) && !empty($campaign['rewards'])) {
                    foreach ($campaign['rewards'] as $single_reward) {
                        if (isset($single_reward->display_name) && !empty($single_reward->display_name)) {
                            $reward .= $single_reward->display_name . ',';
                        }
                    }
                }
            }
    ...
    function changeDisplayDate($campaign)
    {
        if (isset($campaign->start_at) && !empty($campaign->start_at)) {
            $campaign->start_at = self::$woocommerce_helper->beforeDisplayDate($campaign->start_at);
        }
        if (isset($campaign->end_at) && !empty($campaign->end_at)) {
            $campaign->end_at = self::$woocommerce_helper->beforeDisplayDate($campaign->end_at);
        }
        return $campaign;
    }
