...
' . $svg_file . '<p style="margin: 0 0 0;">' . $message . '</p></div>';
    ...
    {
        $action_list = array(
            'admin_change' => sprintf(__('%s changed manually', 'wp-loyalty-rules'), $this->getPointLabel(3)),
            ...
            'custom_note' => __('Custom Note', 'wp-loyalty-rules'),
    ...
    {
        if (!is_array($data) || empty($data['user_email']) || ($data['points'] <= 0 && !in_array($data['action_type'], array('new_user_add', 'admin_change', 'import'))) || empty($data['action_type'])) {
            return false;
        }
        $conditions = array(
            'user_email' => array(
