...
class Base
{
    ...
    public function getCampaigns()
    {
        ...
        $campaign_list = apply_filters('wll_before_launcher_earn_points_data', $campaign_list, $user_email);
    ...
    function getCampaignActions(&$active_campaigns, $user, $woocommerce_helper)
    {
        ...
        switch ($action_type) {
            case 'followup_share':
                ...
                break;
            case 'birthday':
                $active_campaigns->birth_date = isset($user->birthday_date) && !empty($user->birthday_date) && $user->birthday_date != '0000-00-00' ? $user->birthday_date : (isset($user->birth_date) && !empty($user->birth_date) ? $woocommerce_helper->beforeDisplayDate($user->birth_date, "Y-m-d") : '');
