...
class Customers extends Base
{
    function getCustomerList()
    {
        ...
        foreach ($items as &$item) {
            ...
            $item->birthday_date_display_format = empty($item->birthday_date) ? '-' : self::$woocommerce->convertDateFormat($item->birthday_date, 'd/m/Y');
            ...
            $item->created_date = isset($item->created_date) && !empty($item->created_date) ? self::$woocommerce->beforeDisplayDate($item->created_date, 'd/m/Y') : '';
        }
        $data['success'] = true;
        $data['data'] = array(
            'item' => $items,
    ...
    function getCustomer()
    {
        ...
        try {
            ...
            $user->birthday_date_display_format = empty($user->birthday_date) ? '-' : self::$woocommerce->convertDateFormat($user->birthday_date, 'd/m/Y');
            /*$user->birthday_date = !isset($user->birthday_date) || empty($user->birthday_date) ? self::$woocommerce->beforeDisplayDate($user->birth_date, 'Y-m-d') : $user->birthday_date;
            $user->birthday_date_display_format = empty($user->birthday_date) ? '' : self::$woocommerce->convertDateFormat($user->birthday_date,'d/m/Y');*/
            ...
            $user->created_date = isset($user->created_date) && !empty($user->created_date) ? self::$woocommerce->beforeDisplayDate($user->created_date, 'd/m/Y') : '';
            $data['success'] = true;
            $data['data'] = $user;
        } catch (\Exception $e) {
            $data['success'] = false;
    ...
    function updateCustomerBirthday()
    {
        ...
        $action_data = array(
            ...
            'points' => 0,
            'birthday_date' => !empty($birth_date) ? self::$woocommerce->convertDateFormat($birth_date) : null,
    ...
    function getCustomerTransaction()
    {
        ...
        foreach ($items as &$item) {
            ...
            $item->order_link = admin_url('post.php?' . http_build_query(array('post' => $item->order_id, 'action' => 'edit')));
            $item->currency_symbol = get_woocommerce_currency_symbol($item->order_currency);
            $item->created_at = isset($item->created_at) && !empty($item->created_at) ? self::$woocommerce->beforeDisplayDate($item->created_at, 'd/m/Y') : '';
        }
        $data['success'] = true;
        $data['data'] = array(
            'items' => $items,
    ...
    function getCustomerRewards()
    {
        ...
        foreach ($items as $item) {
            $item->action_name = $earn_helper->getActionName($item->action_type);
            $item->end_at_converted = empty($item->end_at) ? '-' : self::$woocommerce->beforeDisplayDate($item->end_at, 'd/m/Y');
            ...
            $item->created_at = self::$woocommerce->beforeDisplayDate($item->created_at, 'd/m/Y');
            $item->currency_symbol = get_woocommerce_currency_symbol($item->reward_currency);
            $item->expire_email_date_converted = empty($item->expire_email_date) ? '-' : self::$woocommerce->beforeDisplayDate($item->expire_email_date, 'd/m/Y');
    ...
    function updateExpiryDates()
    {
        ...
        $update_data = array();
        if (!empty($end_at) && $end_at != '-') {
            $update_data['end_at'] = self::$woocommerce->beforeSaveDate($end_at);
        }
        if (!empty($expire_email_date) && $expire_email_date != '-') {
        ...
        if ($reward_update_status) {
            $log_data = array(
                ...
                'note' => sprintf(__('%s expire date/expire email date updated by admin(%s)', 'wp-loyalty-rules'), $single_user_reward->email, self::$woocommerce->get_email_by_id(get_current_user_id())),
        ...
        $data['success'] = true;
        $data['data'] = array(
            'message' => __('The expiry date has been updated successfully', 'wp-loyalty-rules')
        );
        wp_send_json($data);
    }
}